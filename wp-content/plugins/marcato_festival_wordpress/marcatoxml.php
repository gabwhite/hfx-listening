...
class marcatoxml_plugin {
	...
	public function register_custom_post_types(){
		...
		register_post_type("marcato_artist", array(
			...
			"taxonomies"=>array("Artist Category","artist_tag")
			)
		);
		...
		register_taxonomy( 'artist_tag', array('marcato_artist'), array(
		  'hierarchical' => true,
		  'labels' => array(
		...
		foreach ($xml->artist as $artist) {
			...
			if ($this->options["include_meta_data"]=="1"){
				...
				if(!empty($artist->{'custom-fields'}) ){
					$i = 0;
					foreach ($artist->{'custom-fields'}->{'custom-field'} as $artstra){
						$post_meta["marcato_artist_custom_" . $i . "_value"] = $artstra->value;
						$i++;
					}
